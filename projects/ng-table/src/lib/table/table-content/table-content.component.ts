...
export class TableContentComponent implements OnInit, OnChanges, OnDestroy {
  ...
  @Output()
  changeOrderState: EventEmitter<SortState> = new EventEmitter<SortState>();
  ...
  initPinnedColumnsSetting() {
    ...
    this.pinnedColumnsWidth = this.pinnedColumns.reduce(
      (pre, cur): number => pre + parseInt(cur.width),
      0
    );
    let left = 0;
  ...
  clearExpandedRow() {
    if (
      this.rowExpandTemplate &&
      this.tbodyElement &&
      this.tbodyElement.nativeElement
    ) {
      if (this._viewRefs) {
        this._viewRefs.forEach((item) => this._appRef.detachView(item));
      }
      const expandedRows = this.tbodyElement.nativeElement.querySelectorAll(
        "row-expanded"
      );
      if (expandedRows) {
        expandedRows.forEach((item) =>
          this.tbodyElement.nativeElement.removeChild(item)
        );
      }
    }
  }
  registerEvent() {
    this.columnTriggerExpand$
      .pipe(takeUntil(this._unsubscribe$))
      .subscribe(({ rowData, tRowEl }) => {
        this.tableRowClickHandler(rowData, tRowEl, true);
      });
  }
  sort(column, e: Event) {
    if ((e.currentTarget as HTMLElement).classList.contains("sortable")) {
      ...
      // update direction
      if (currentSortColumn === column.columnDef) {
        this.sortState.order =
          this.sortState.order === SortOrder.Asc
            ? SortOrder.Desc
            : SortOrder.Asc;
      }
      // update sort column name
      else {
        this.sortState.columnDef = column.columnDef;
  ...
  tableRowClickHandler(
    ...
    if (!ignoreSettings) {
      if (
        !this.settings.rows.triggerExpand &&
        !this.settings.rows.triggerExpandFunc
      )
        return;
    }
    const rowEmbeddedView = this.rowExpandTemplate.templateRef.createEmbeddedView(
      { rowData }
    );
  ...
  handleCollapseRow(tRow: HTMLElement, templateEl: HTMLElement) {
    ...
    if (
      tRow.nextElementSibling &&
      tRow.nextElementSibling.attributes.getNamedItem("row-expanded")
    ) {
